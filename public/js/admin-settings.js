...
async function loadSettings() {
    try {
        ...
        loadDataFormatsTab(data.dataFormats);
    } catch (e) {
        console.error(e);
...
async function saveSettings(e) {
    ...
    const dataFormats = collectDataFormats();
    const partial = {
        mail: { ...mail },
        features,
        announcements,
...
function loadDataFormatsTab(dataFormats) {
    ...
    setVal("df-pl-sheetnames", sn && typeof sn === "object" && !Array.isArray(sn) && Object.keys(sn).length ? JSON.stringify(sn, null, 2) : "");
    ...
    setVal("df-est-aliases", est && typeof est === "object" && !Array.isArray(est) && Object.keys(est).length ? JSON.stringify(est, null, 2) : "");

    const fix = df.logisticsFixedColumnImport || {};
    setVal("df-fixed-cols", fix && typeof fix === "object" && !Array.isArray(fix) && Object.keys(fix).length ? JSON.stringify(fix, null, 2) : "");
}
...
function collectDataFormats() {
    ...
    if (typeof sheetNamesByCategory !== "object" || sheetNamesByCategory === null || Array.isArray(sheetNamesByCategory)) {
        sheetNamesByCategory = {};
    }
    ...
    if (typeof estimateImportAliases !== "object" || estimateImportAliases === null || Array.isArray(estimateImportAliases)) {
        estimateImportAliases = {};
    }
    ...
    if (logisticsFixedColumnImport && typeof logisticsFixedColumnImport === "object" && !Array.isArray(logisticsFixedColumnImport)) {
