...
| **テスト数** | 453〜454 |
| **行カバレッジ（全体）** | 約 87% |
| **分岐カバレッジ（全体）** | 約 68〜69%（ばらつきあり） |
| **jest 閾値（global）** | lines: 80, branches: 67 |
...
| 観点 | 現状 | 外販レベルで欲しい水準 |
|------|------|------------------------|
| カバレッジ | 分岐 68〜69%、閾値 80/67 | 分岐 70% 以上、閾値 80/70 |
...
- [ ] 全体 行 80% 以上・分岐 70% 以上（複数回実行で安定）
- [ ] `coverageThreshold.global` を 80/70 に更新
